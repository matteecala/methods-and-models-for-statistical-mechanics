...
Superoperators $\Phi:B(\mathcal{H})\rightarrow B(\mathcal{H})$ are maps between operators. \newline Simple positivity — $X \geq0\Rightarrow \Phi(X)\geq0$ — is not sufficient for quantum-mechanical consistency: the transpose map on $B(\mathbb{C}^2)$ is positive but not 2-positive.\newline The correct notion is that of \textbf{complete positivity}. The Kraus-Stinespring theorem characterizes CP maps via the decomposition $\Phi(X)=\sum_jV_jXV_j^*$. A quantum channel is a CP unital map, whose adjoint operates in the Schrödinger picture.
...
The singular values of $A$ are the eigenvalues of $|A| = \sqrt{A^*A}$. \newline The Schatten-$p$ norm is $\|A\|_p = (\mathrm{tr}|A|^p)^{1/p}$. The notable cases are: $p=1$ the trace norm, $p=2$ the Hilbert-Schmidt norm with inner product, $p=\infty$ the operator norm. The function $p \mapsto \|A\|_p$ is real analytic and decreasing, with limit $\|A\|_\infty$ as $p \to \infty$.
...
The Peierls-Bogolyubov inequality is the quantum analogue of Jensen's inequality for exponential functions, and finds application in free energy bounds. \newline Klein's inequality — $\mathrm{tr}(A \log A - A \log B) \geq \mathrm{tr}(A - B)$ — is the foundation of quantum relative entropy $S(\rho \| \sigma) = \mathrm{tr}(\rho \log \rho - \rho \log \sigma) \geq 0$, with equality if and only if $A = B$.
